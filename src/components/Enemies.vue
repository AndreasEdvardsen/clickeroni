<template>
  <v-card height="100%" elevation="0">
    <v-card-title>Enemies</v-card-title>
      ...
      <v-card class="d-flex" style="user-select: none;" dark v-bind:color="item.color" @click="itemClicked(item)" v-bind:disabled="HITPOINTS < 10">
        <v-tooltip color="black" left>
          <template v-slot:activator="{on, attrs}">
            <v-card v-bind="attrs" v-on="on" flat width="110px">
          ...
          <h3 class="mt-2">Loot Drop:</h3>
          <div v-for="loot in item.loot" :key="loot" :class="loot.color + '--text'">{{ loot.label }}: {{ loot.value }}</div>
        </v-tooltip>
        <div>
